...
def prepare_vehicle_data(
    ...
    pdf = df_clean.select(metadata_cols + feature_cols).toPandas()
    ...
    mean = np.mean(X_raw[train_idx], axis=0)
    std = np.std(X_raw[train_idx], axis=0)
    std[std == 0.0] = 1.0
    ...
    print(f"Full tensor shape:  {X_full_tensor.shape}")
...
def train_autoencoder(
    ...
    """
    Train one autoencoder for a specific year and save its best checkpoint.
    """
    ...
    # All models are saved in the same folder
    model_dir = Path("models/vehicle_autoencoders")
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        mode="min",
        save_top_k=1,
        filename=f"vehicle_autoencoder_{year}",
    ...
    if best_val_loss is not None:
        best_val_loss = best_val_loss.detach().cpu().item()

    return trainer, checkpoint_callback.best_model_path, best_val_loss
...
for year in years:
    ...
    print(f"Raw rows for {year}: {n_rows}")
    ...
    try:
        ...
        print(f"Could not prepare data for {year}: {e}")
        continue

    n_samples = len(metadata)

    print(f"Usable observations for {year}: {n_samples}")

    if n_samples < 2:
        print(f"Not enough observations for {year}. Skipping.")
        continue

    scaler_path = model_dir / f"scaler_{year}.pt"
    ...
    trainer, best_model_path, best_val_loss = train_autoencoder(
        model=model,
...
for year, result in training_results.items():
    print(
        f"{year}: "
        f"{result['n_samples']:,} observations | "
